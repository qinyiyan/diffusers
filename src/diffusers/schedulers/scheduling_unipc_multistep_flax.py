...
class FlaxUniPCMultistepScheduler(FlaxSchedulerMixin, ConfigMixin):
  ...
  def set_timesteps(
    ...
    return state.replace(
        ...
        step_index=-1,
        begin_index=-1,
        last_sample=jnp.zeros(shape, dtype=self.dtype),
        this_order=0,
    )
  ...
  def _init_step_index(
    ...
    state = jax.lax.cond(
        state.begin_index == -1,
        lambda _: state.replace(step_index=self.index_for_timestep(state, timestep)),
        lambda _: state.replace(step_index=state.begin_index),
        operand=None,
    )
    return state
  ...
  def step(
    ...
    state = jax.lax.cond(
        state.step_index == -1,
        lambda _: self._init_step_index(state, timestep_scalar),
        lambda _: state,
        operand=None,
    )
